...
INTENSITIES1065_STEPS = 5
INTENSITIES1065 = np.linspace(2, 8, INTENSITIES1065_STEPS) * kWpercm2


min_max_transition_energy_over_1065 = np.zeros((INTENSITIES1065_STEPS, 29300))
gate_time_over_1065_us = np.zeros((INTENSITIES1065_STEPS, 29300))
for iiii, INTEN1065 in enumerate(INTENSITIES1065):
    ...
    INTEN817_MIN = 0.95 * INTEN817_MAGIC
    INTEN817_MAX = 1.05 * INTEN817_MAGIC
    INTEN817 = np.linspace(INTEN817_MIN, INTEN817_MAX, INTEN817_STEPS)
    MAGIC_INDEX = int((INTEN817_STEPS-1)/2)

    print(INTEN817_MAGIC, INTEN817[MAGIC_INDEX])
    ...
    min_max_transition_energy_per_transition = np.zeros((generated_edge_labels.shape[0]))
    gate_time_per_transition = np.zeros((generated_edge_labels.shape[0]))
    # T_CUM_GET_US = np.zeros((generated_edge_labels.shape[0]))
    for i, label_pair in enumerate(generated_edge_labels):
        ...
        T_RABI = (0.2*T_G_UNPOL[i,MAGIC_INDEX] + 0.8*T_G_POL[i,MAGIC_INDEX])*1e6*10**(2/2)
        T_CUM_GET_US = (0.3 * cumulative_unpol_fidelity_from_initials[from_node_index, MAGIC_INDEX] + 0.7 * cumulative_pol_fidelity_from_initials[from_node_index, MAGIC_INDEX])*1e6*10**(2/2)

        if T_CUM_GET_US > 300 or T_RABI > 100:
            min_max_transition_energy_per_transition[i] = -1
            continue
        ...
        min_max_transition_energy = np.max(transition_energy) - np.min(transition_energy)
        min_max_transition_energy_per_transition[i] = min_max_transition_energy
        gate_time_per_transition[i] = T_RABI

        ax.plot(INTEN817, transition_energy)
    ax.set_title(f"{iiii}")
    plt.show()
    min_max_transition_energy_over_1065[iiii] = min_max_transition_energy_per_transition
    gate_time_over_1065_us[iiii] = gate_time_per_transition
...
for psi, label_pair_d in enumerate(generated_edge_labels):
    min_max_intensity = min_max_transition_energy_over_1065[:,psi]
    gate_time = gate_time_over_1065_us[:,psi]
    if np.min(min_max_intensity) >= 0 and label_pair_d[3]>label_pair_d[0] and np.max(min_max_intensity) < 4 * kHz:
        pol_index = label_pair_d[4]-label_pair_d[1]
        c='k'
        if pol_index == 0:
            c='blue'
        elif pol_index == 2:
            ...
            c='red'
        ax.plot(INTENSITIES1065/kWpercm2, min_max_intensity/kHz, c=c, alpha=0.8)
        loc = [INTENSITIES1065[-1]/kWpercm2, min_max_intensity[-1]/kHz]
        if (label_pair_d == (0,8,0,1,8,3)).all():
            print(f"{gate_time[1]}us")
            loc[1] -= 0.18
        elif (label_pair_d == (0,6,1,1,8,3)).all():
            print(f"{gate_time[1]}us")
            loc[1] -= 0.36
        elif (label_pair_d == (1,6,4,2,8,5)).all():
            print(f"{gate_time[1]}us")
        ax.text(*loc, rf"$({label_pair_d[0]},{label_pair_d[1]//2})_{label_pair_d[2]} \rightarrow ({label_pair_d[3]},{label_pair_d[4]//2})_{label_pair_d[5]}$", c=c).set_clip_on(True)



# for psi, min_max_intensity in enumerate(min_max_transition_energy_over_1065.T):
#     if np.min(min_max_intensity) >= 0:
#         print(generated_edge_labels[psi])
#         ax.plot(INTENSITIES1065/kWpercm2, min_max_intensity/kHz, c='k', alpha=0.1)

ax.set_ylabel(
r"""$\pm 5\%$ tweezer intensity max-min
energy level fluc at magic 817 (kHz)
""")
ax.set_xlabel(r"I1065 (kw/cm^2)")
ax.axhline(1, c='k', lw=1, linestyle='--')
